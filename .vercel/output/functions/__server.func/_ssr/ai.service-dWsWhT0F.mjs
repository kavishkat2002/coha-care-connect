import { c as doctors, i as AI_DISCLAIMER, u as init_mock } from "./server-DlLVIWDa.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/ai.service-dWsWhT0F.js
init_mock();
...
Return ONLY a valid JSON object matching this strict structure (and absolutely no other text or markdown tags):
{
  ...
  "boundingBox": [x, y, width, height] (Array of 4 numbers between 0.0 and 1.0 representing the bounding box of the primary lesion)
	...
	await delay(1400);
	if (region.toLowerCase() === "skin") return {
		quality: "Good",
		region: "Skin",
		lesionsDetected: 1,
		risk: "elevated",
		confidence: 89,
		explanation: "Digital pathology analysis detected a prominent central lesion exhibiting marked asymmetry, irregular notched borders, variegated erythematous pigmentation, and central ulceration. Clinical threshold of 0.23 was significantly exceeded, indicating elevated risk for malignant skin lesion according to ISIC dermoscopic criteria.",
		plainLanguageExplanation: "The AI scan identified an irregular skin lesion with central ulceration (crusting/bleeding), varied coloring, and uneven borders. Because these features are concerning for skin cancer, we strongly advise scheduling a dermatologist appointment as soon as possible for a professional evaluation.",
		recommendation: [
	...
	return {
		...
		risk: "low",
		confidence: 81,
		explanation: "A single well-demarcated area was highlighted. Its borders appear regular and colour distribution is even, which is typical of benign changes.",
		plainLanguageExplanation: "We found one spot in your image. It looks like it has a clear shape with even colouring, which is usually a sign that it's nothing to worry about. To be safe, keep an eye on it for the next two weeks and take another photo if anything changes.",
		recommendation: ["Monitor the area for 14 days and re-capture an image", `Book a ${region.toLowerCase()} specialist review if it grows or changes colour`],
