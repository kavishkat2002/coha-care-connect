import { c as doctors, i as AI_DISCLAIMER, u as init_mock } from "./server-B024QcWx.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/ai.service-DmJX46uV.js
init_mock();
...
function analyzeUploadedImageFeatures(imageBase64, region = "Skin") {
	...
	if (imageBase64 && imageBase64.length > 100) {
		...
		for (let idx = 0; idx < samples.length; idx++) {
			...
			seed = seed * 31 + charCode & 2147483647;
		}
		seed = seed + len & 2147483647;
	} else seed = Math.floor(Math.random() * 1e4);
	const posHash = Math.abs(seed);
	if (region.toLowerCase() === "skin") {
		const profile = posHash % 3;
		if (profile === 0) {
			const prob = 76 + posHash % 18;
			const diameterVal = (7.5 + posHash % 28 / 10).toFixed(1);
			const conf = 87 + posHash % 10;
			const xBox = .35 + posHash % 15 / 100;
			const yBox = .2 + posHash % 15 / 100;
			return {
				quality: "Good",
				region: "Skin",
				...
				risk: "elevated",
				confidence: conf,
				explanation: `Vision AI analysis detected an asymmetrical skin lesion with irregular notched margins, variegated color distribution (dark brown, black, and erythematous areas), and focal central ulceration. Malignancy probability of ${prob}% significantly exceeds the 0.23 clinical sensitivity threshold.`,
			...
			const prob = 26 + posHash % 15;
			const diameterVal = (5 + posHash % 16 / 10).toFixed(1);
			const conf = 82 + posHash % 11;
			const xBox = .32 + posHash % 18 / 100;
			const yBox = .25 + posHash % 15 / 100;
			return {
				quality: "Good",
				region: "Skin",
				lesionsDetected: 1,
				risk: "moderate",
				confidence: conf,
				explanation: `Vision AI assessment highlighted an atypical skin spot with mild border irregularity and light tan-to-brown pigmentation. Dermoscopic patterns suggest a benign seborrheic keratosis or dysplastic nevus. Malignancy probability is estimated at ${prob}%.`,
			...
			const prob = 5 + posHash % 12;
			const diameterVal = (2.8 + posHash % 18 / 10).toFixed(1);
			const conf = 90 + posHash % 8;
			const xBox = .4 + posHash % 10 / 100;
			const yBox = .32 + posHash % 10 / 100;
			return {
				quality: "Good",
				region: "Skin",
				lesionsDetected: 1,
				risk: "low",
				confidence: conf,
				explanation: `A single well-demarcated area was highlighted. Its borders appear regular, symmetrical, and color distribution is uniform light brown (${prob}% malignancy probability), characteristic of a benign melanocytic nevus.`,
