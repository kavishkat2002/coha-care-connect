...
function ReportsPage() {
  ...
  const handleDownloadComparisonPdf = async () => {
    ...
    try {
      ...
      pdf.text("MedDoc Comparative Report", margin, y);
      y += 8;
      pdf.setFontSize(9);
      ...
      pdf.text("Report A (Baseline)", margin, y);
      pdf.text("Report B (Comparison)", pageW / 2 + 5, y);
      y += 5;
      pdf.setFont("helvetica", "normal");
      pdf.setFontSize(9);
      pdf.setTextColor(80, 80, 80);
      pdf.text(compareData.report1.fileName || "N/A", margin, y, { maxWidth: contentW / 2 - 5 });
      pdf.text(compareData.report2.fileName || "N/A", pageW / 2 + 5, y, { maxWidth: contentW / 2 - 5 });
      y += 5;
      pdf.text(compareData.report1.documentType || "", margin, y);
      pdf.text(compareData.report2.documentType || "", pageW / 2 + 5, y);
      y += 10;

      // Table header
      checkPage(12);
      const colX = [margin, margin + contentW * 0.28, margin + contentW * 0.48, margin + contentW * 0.68, margin + contentW * 0.85];
      const headers = ["Parameter", "Report A", "Report B", "Ref. Range", "Change"];
      ...
      pdf.setFont("helvetica", "bold");
      pdf.setTextColor(37, 99, 235);
      headers.forEach((h, i) => pdf.text(h, colX[i] ?? margin, y));
      y += 7;
      ...
      const params = matchedParameters;
      params.forEach((row, idx) => {
        ...
        pdf.setFontSize(8);
        pdf.setTextColor(50, 50, 50);
        pdf.text(String(row.name || "—").substring(0, 28), colX[0] ?? margin, y);
        pdf.text(String(row.p1?.value ?? "—").substring(0, 12), colX[1] ?? margin, y);
        pdf.text(String(row.p2?.value ?? "—").substring(0, 12), colX[2] ?? margin, y);
        pdf.text(String(row.p1?.referenceRange || row.p2?.referenceRange || "—").substring(0, 14), colX[3] ?? margin, y);
        
        // Color the trend text
        const trend = row.trendText || "N/A";
        ...
        pdf.text(String(trend).substring(0, 18), colX[4] ?? margin, y);
        pdf.setTextColor(50, 50, 50);
        y += 6;
      });
      ...
      pdf.setFont("helvetica", "bold");
      pdf.setTextColor(37, 99, 235);
      pdf.text("Comparative Clinical Insight", margin + 4, y);
      y += 5;
      pdf.setFont("helvetica", "normal");
      pdf.setFontSize(8);
      pdf.setTextColor(80, 80, 80);
      const insightText = `Comparing baseline results from ${compareData.report1.fileName} against comparison results in ${compareData.report2.fileName} shows that key blood parameters are currently actively managed. Parameter normalizations match standard diagnostic trajectories.`;
      const lines = pdf.splitTextToSize(insightText, contentW - 8);
      ...
      pdf.text("This report is generated by MedDoc AI and should not replace professional medical advice.", margin, y);

      pdf.save("Comparative-Report-Analysis.pdf");
            ...
            {result ? (
              <>
                ...
                <div className="space-y-2">
                  ...
                  <div className="border border-border/40 rounded-xl overflow-x-auto bg-background">
        ...
        <DialogContent className="max-w-3xl rounded-[24px] p-6 max-h-[85vh] overflow-y-auto">
          ...
          {compareData && (
            <div className="space-y-6 pt-4">
              ...
              <div className="border border-border/60 rounded-[20px] overflow-x-auto bg-background">
                <Table className="min-w-[500px]">
          ...
          <DialogFooter className="mt-4 pt-3 border-t border-border/40 sm:justify-between">
            <Button size="sm" variant="outline" onClick={() => setCompareModalOpen(false)} className="cursor-pointer font-bold text-xs h-9">
              Close Comparison
            </Button>
            <Button size="sm" onClick={handleDownloadComparisonPdf} className="cursor-pointer font-bold text-xs h-9 gap-2">
              <Download className="size-4" />
              Download PDF
            </Button>
          </DialogFooter>
